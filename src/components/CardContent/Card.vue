...
<template>
	<div class="card" :class="{'shadow-2xl border-none': isActive}" @click="setActiveHandler">
		...
		<div class="card__info">
			<div>
				<h6 class="thin">{{dateLabel}}</h6>
				<span class="boldie">{{item.startDate}}</span>
			</div>
			<div v-show="!notOnHold">
				<h6 class="thin">Status</h6>
				<Status class="boldie" :status="props.item.status" />
			</div>
			<div class="summary">
				<div class="summary__badge">
					...
					<span class="thin">Tasks</span>
				</div>
				<div class="summary__badge">
					<span class="stat">
						{{item.users}}
					</span>
					<span class="thin">Members</span>
				</div>
			</div>
		</div>
		<div class="card__members ">
			<div class="header">
				<span class="boldie">Members</span>
			</div>
			<div class="images">
		...
		<div class="progress">
			<div class="progress__header">
				<span class="boldie">Progress</span>
				<span class="thin">{{progressWidth}}</span>
			</div>
...
.card {
	...
	&__info {
		...
		.summary {
			@apply grid grid-cols-2 p-1 py-3.5 bg-gray-light text-center divide-x divide-divider w-[148px] rounded-md;
